...
func SetupControllers(ctx context.Context, wg *sync.WaitGroup, mgr ctrl.Manager, config *rest.Config, opts *options.Options) error { //nolint:gocyclo
	...
	resourceSnapshotResolver := controller.NewResourceSnapshotResolver(mgr.GetClient(), mgr.GetScheme()).WithConfig(
		controller.NewResourceSnapshotConfig(opts.ResourceSnapshotCreationMinimumInterval, opts.ResourceChangesCollectionDuration))
	pc := &placement.Reconciler{
		...
		ResourceSnapshotResolver: *resourceSnapshotResolver.WithDefaultSnapshotFunction(),
	}
