...
// Note that this container will run in parallel with other containers.
var _ = Describe("placing wrapped resources using CRP NamespaceOnly then ResourcePlacement", func() {
	Context("Test NamespaceOnly placement through CRP then resource placement through ResourcePlacement", Ordered, func() {
		crpName := fmt.Sprintf(crpNameTemplate, GinkgoParallelProcess())
		...
		It("should update CRP status to show namespace placement", func() {
			...
			statusUpdatedActual := crpStatusUpdatedActual(wantSelectedResources, allMemberClusterNames, nil, "0")
		...
		It("should place only the namespace on all member clusters", func() {
			for idx := range allMemberClusters {
				...
				nameSpaceResourceNotPlacedActual := namespacedResourcesRemovedFromClusterActual(memberCluster)
		...
		It("should update ResourcePlacement status as expected", func() {
			...
			Eventually(rpStatusUpdatedActual(rpSelectedResources, allMemberClusterNames, nil, "0"), workloadEventuallyDuration, eventuallyInterval).Should(Succeed(), "Failed to update ResourcePlacement status as expected")
		})
		...
		It("should place the enveloped resources on all member clusters", func() {
			for idx := range allMemberClusters {
				memberCluster := allMemberClusters[idx]
				workResourcesPlacedActual := checkAllResourcesPlacement(memberCluster)
				Eventually(workResourcesPlacedActual, workloadEventuallyDuration, eventuallyInterval).Should(Succeed(), "Failed to place enveloped resources on member cluster %s", memberCluster.ClusterName)
			}
		})
