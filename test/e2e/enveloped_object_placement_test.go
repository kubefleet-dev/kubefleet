					...
					PerClusterPlacementStatuses: []placementv1beta1.PerClusterPlacementStatus{
						{
							...
							ObservedResourceIndex: "0",
							Conditions:            perClusterResourcePlacementWorkSynchronizedFailedConditions(crp.Generation, false),
						},
					},
	...
	return func() error {
		...
		for _, placementStatus := range crp.Status.PerClusterPlacementStatuses {
			...
			if len(placementStatus.FailedPlacements) != 0 {
				...
				if diff := cmp.Diff(placementStatus.Conditions, resourcePlacementApplyFailedConditions(crp.Generation), placementStatusCmpOptions...); diff != "" {
					return fmt.Errorf("CRP status diff (-got, +want): %s", diff)
				}
			} else {
				// the cluster is stuck behind a rollout schedule since we now have 1 cluster that is not in applied ready status
				if diff := cmp.Diff(placementStatus.Conditions, resourcePlacementSyncPendingConditions(crp.Generation), placementStatusCmpOptions...); diff != "" {
					return fmt.Errorf("CRP status diff (-got, +want): %s", diff)
				}
			}
		}
