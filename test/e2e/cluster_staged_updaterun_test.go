...
var _ = Describe("test CRP rollout with staged update run", func() {
	...
	Context("Test resource rollout and rollback with staged update run", Ordered, func() {
		...
		It("Should create a cluster staged update run successfully", func() {
			createClusterStagedUpdateRunSucceed(updateRunNames[0], crpName, resourceSnapshotIndex1st, strategyName, placementv1beta1.StateExecute)
		})
		...
		It("Should create a new cluster staged update run successfully", func() {
			createClusterStagedUpdateRunSucceed(updateRunNames[1], crpName, resourceSnapshotIndex2nd, strategyName, placementv1beta1.StateExecute)
		})
		...
		It("Should create a new staged update run with old resourceSnapshotIndex successfully to rollback", func() {
			createClusterStagedUpdateRunSucceed(updateRunNames[2], crpName, resourceSnapshotIndex1st, strategyName, placementv1beta1.StateExecute)
		})
	...
	Context("Test cluster scale out and shrink using pickFixed policy with staged update run", Ordered, func() {
		...
		It("Should create a cluster staged update run successfully", func() {
			createClusterStagedUpdateRunSucceed(updateRunNames[1], crpName, resourceSnapshotIndex1st, strategyName, placementv1beta1.StateExecute)
		})
	...
	Context("Test cluster staged update run with overrides", Ordered, func() {
		...
		It("Should create a cluster staged update run successfully", func() {
			createClusterStagedUpdateRunSucceed(updateRunName, crpName, resourceSnapshotIndex1st, strategyName, placementv1beta1.StateExecute)
		})
	...
	Context("Test CRP rollout strategy transition from rollingUpdate to external", Ordered, func() {
		...
		It("Create a staged update run with new resourceSnapshotIndex and verify rollout happens", func() {
			createClusterStagedUpdateRunSucceed(updateRunName, crpName, resourceSnapshotIndex2nd, strategyName, placementv1beta1.StateExecute)
	...
	Context("Test resource rollout with staged update run by update run states - (Initialize -> Execute)", Ordered, func() {
		...
		It("Should rollout resources to member-cluster-2 only after update run is in Execute state", func() {
			...
			updateClusterStagedUpdateRunState(updateRunNames[0], placementv1beta1.StateExecute)
...
func updateClusterStagedUpdateRunState(updateRunName string, state placementv1beta1.State) {
	updateRun := &placementv1beta1.ClusterStagedUpdateRun{}
	Expect(hubClient.Get(ctx, types.NamespacedName{Name: updateRunName}, updateRun)).To(Succeed(), "Failed to get ClusterStagedUpdateRun %s", updateRunName)

	updateRun.Spec.State = state
	Expect(hubClient.Update(ctx, updateRun)).To(Succeed(), "Failed to update ClusterStagedUpdateRun %s", updateRunName)
}
