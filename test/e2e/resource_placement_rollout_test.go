...
var _ = Describe("placing namespaced scoped resources using a RP with rollout", func() {
...
var _ = Describe("placing namespaced custom resources using a RP with rollout", Ordered, func() {
	crpName := fmt.Sprintf(crpNameTemplate, GinkgoParallelProcess())
	rpName := fmt.Sprintf(rpNameTemplate, GinkgoParallelProcess())
	rpKey := types.NamespacedName{Name: rpName, Namespace: appNamespace().Name}
