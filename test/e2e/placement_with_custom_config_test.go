...
var _ = Describe("validating that CRP status can be updated after updating the resources when using customized resourceSnapshotCreationInterval", Label("custom"), Ordered, func() {
	// skip entire suite if interval is zero
	BeforeAll(func() {
		if resourceSnapshotCreationInterval == 0 {
			Skip("Skipping customized-config placement test when RESOURCE_SNAPSHOT_CREATION_INTERVAL=0m")
	...
	It("should update CRP status for snapshot 1 as expected", func() {
		crpStatusUpdatedActual := crpStatusUpdatedActual(workResourceIdentifiers(), allMemberClusterNames, nil, "1")
		Eventually(crpStatusUpdatedActual, resourceSnapshotCreationInterval+eventuallyDuration, eventuallyInterval).Should(Succeed(), "Failed to update CRP %s status as expected", crpName)
	})
