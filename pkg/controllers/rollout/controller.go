...
func (r *Reconciler) Reconcile(ctx context.Context, req runtime.Request) (runtime.Result, error) {
	...
	latestResourceSnapshot, err := controller.FetchMasterResourceSnapshot(ctx, r.UncachedReader, string(placementKey))
	if err != nil {
		klog.ErrorS(err, "Failed to find the latest resourceSnapshot for the placement",
			"placement", placementObjRef)
		return runtime.Result{}, err
	}
	klog.V(2).InfoS("Found the latest resourceSnapshot for the placement", "placement", placementObjRef, "latestResourceSnapshot", klog.KObj(latestResourceSnapshot))
	...
	matchedCRO, matchedRO, err := overrider.FetchAllMatchingOverridesForResourceSnapshot(ctx, r.Client, r.InformerManager, string(placementKey), latestResourceSnapshot)
	if err != nil {
	...
	toBeUpdatedBindings, staleBoundBindings, upToDateBoundBindings, needRoll, waitTime, err := r.pickBindingsToRoll(ctx, allBindings, latestResourceSnapshot, placementObj, matchedCRO, matchedRO)
	if err != nil {
...
func (r *Reconciler) checkAndUpdateStaleBindingsStatus(ctx context.Context, bindings []fleetv1beta1.BindingObj) error {
	...
	for i := 0; i < len(bindings); i++ {
		...
		// Access conditions through the binding status
		bindingStatus := binding.GetBindingStatus()
		var rolloutStartedCondition *metav1.Condition
		for _, cond := range bindingStatus.Conditions {
			if cond.Type == string(fleetv1beta1.ResourceBindingRolloutStarted) {
				rolloutStartedCondition = &cond
				break
			}
		}
...
func createUpdateInfo(binding fleetv1beta1.BindingObj,
	latestResourceSnapshot fleetv1beta1.ResourceSnapshotObj, cro []string, ro []fleetv1beta1.NamespacedName) toBeUpdatedBinding {
	desiredBinding := binding.DeepCopyObject().(fleetv1beta1.BindingObj)

	// Get current spec and update it
	currentSpec := desiredBinding.GetBindingSpec()
	currentSpec.State = fleetv1beta1.BindingStateBound
	currentSpec.ResourceSnapshotName = latestResourceSnapshot.GetName()

	// Apply strategy is updated separately for all bindings.

	// TODO: check the size of the cro and ro to not exceed the limit
	currentSpec.ClusterResourceOverrideSnapshots = cro
	currentSpec.ResourceOverrideSnapshots = ro

	// Set the updated spec back
	desiredBinding.SetBindingSpec(*currentSpec)
	...
	for idx := range allBindings {
		...
		case fleetv1beta1.BindingStateBound:
			...
			if binding.GetDeletionTimestamp().IsZero() {
				...
				if bindingSpec.ResourceSnapshotName != latestResourceSnapshot.GetName() || !equality.Semantic.DeepEqual(bindingSpec.ClusterResourceOverrideSnapshots, cro) || !equality.Semantic.DeepEqual(bindingSpec.ResourceOverrideSnapshots, ro) {
...
func (r *Reconciler) calculateRealTarget(placementObj fleetv1beta1.PlacementObj, schedulerTargetedBinds []fleetv1beta1.BindingObj) int {
	...
	if placementSpec.Policy != nil {
		...
		// If policy is nil, fall back to PickAll behavior
		targetNumber = len(schedulerTargetedBinds)
	}
	return targetNumber
}
...
func isBindingReady(binding fleetv1beta1.BindingObj, readyTimeCutOff time.Time) (time.Duration, bool) {
	...
	bindingStatus := binding.GetBindingStatus()
	diffReportCondition := meta.FindStatusCondition(bindingStatus.Conditions, string(fleetv1beta1.ResourceBindingDiffReported))
	...
	availableCondition := meta.FindStatusCondition(bindingStatus.Conditions, string(fleetv1beta1.ResourceBindingAvailable))
...
func (r *Reconciler) updateBindings(ctx context.Context, bindings []toBeUpdatedBinding) error {
	...
	for i := 0; i < len(bindings); i++ {
		...
		switch currentBindingSpec.State {
		...
		case fleetv1beta1.BindingStateBound:
			errs.Go(func() error {
				...
				desiredSpec := binding.desiredBinding.GetBindingSpec()
				klog.V(2).InfoS("Updated a binding to the latest resource", "binding", bindObj, "spec", desiredSpec)
...
func handleResourceSnapshot(snapshot client.Object, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	...
	placement := snapshot.GetLabels()[fleetv1beta1.CRPTrackingLabel]
	if len(placement) == 0 {
	...
	q.Add(reconcile.Request{
		NamespacedName: types.NamespacedName{Name: placement, Namespace: snapshot.GetNamespace()},
	})
}
...
func enqueueResourceBinding(binding client.Object, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	...
	// get the placement name from the label
	placement := binding.GetLabels()[fleetv1beta1.CRPTrackingLabel]
	if len(placement) == 0 {
		// should never happen
		klog.ErrorS(controller.NewUnexpectedBehaviorError(fmt.Errorf("cannot find CRPTrackingLabel label value")),
	...
	// enqueue the CRP to the rollout controller queue
	q.Add(reconcile.Request{
		NamespacedName: types.NamespacedName{Name: placement, Namespace: binding.GetNamespace()},
	})
}
...
func handleResourceBindingUpdated(objectOld, objectNew client.Object, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	...
	for _, conditionType := range conditionsToMonitor {
		// Access conditions through interface methods
		oldBindingStatus := oldBinding.GetBindingStatus()
		newBindingStatus := newBinding.GetBindingStatus()
		oldCond := meta.FindStatusCondition(oldBindingStatus.Conditions, conditionType)
		newCond := meta.FindStatusCondition(newBindingStatus.Conditions, conditionType)
...
func (r *Reconciler) updateBindingStatus(ctx context.Context, binding fleetv1beta1.BindingObj, rolloutStarted bool) error {
	...
	// Get current status using interface methods
	currentStatus := binding.GetBindingStatus()
	// Update conditions directly on the status
	meta.SetStatusCondition(&currentStatus.Conditions, cond)

	// Update the binding status using the Kubernetes client
...
func (r *Reconciler) processApplyStrategyUpdates(
	...
	for idx := range allBindings {
		...
		bindingSpec := binding.GetBindingSpec()
		if equality.Semantic.DeepEqual(bindingSpec.ApplyStrategy, applyStrategy) {
