		...
		if gvk == utils.NamespaceGVK {
			...
			if placementKey.Namespace != "" {
				...
				if isClusterScoped {
					...
					// Namespace-scoped resource: must have selective namespace from 1st pass
					if selectedNamespace != "" {
						// Create a namespaced key with the selective namespace
						...
						// Error: can't select namespace-scoped resources in CRP without a selected namespace
						err := fmt.Errorf("invalid clusterResourcePlacement %s: cannot select namespace-scoped resource %s/%s without a selected namespace (either missing NamespaceWithResourceSelectors mode or namespace selector matched no namespaces)", placementKey.Name, selector.Group, selector.Kind)
						klog.ErrorS(err, "Invalid resource selector for CRP", "selector", selector, "placement", placementKey)
						return nil, NewUserError(err)
					}
				}
			}
